...
class PopAuth(Popup):
    def __init__(self, **kwargs):
        ...
        self.email = BoxLayout(orientation="horizontal", size_hint=(1, None), height=30)
        self.email.add_widget(Label(text="email: ", size_hint=(None, 1), width=30))
...
class Encabezado(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # EXIT
        self.exit = Button(text="X", font_size=20, size_hint=(None,1), width=20)
...
class AssetManApp(App):
    ...
    def build(self):
        ...
        try:
            ...
            #return BoxLayout().add_widget(Label(text=str(e)))#.add_widget(Salir().bind(on_press=self.stop))
        ...
        def addMenu(folder):
            ...
            info = json.dumps(info_json, indent=2, sort_keys=True, separators=[",",": "])
        ...
        #menu = FloatLayout()
        #manager = contenedorInterno(size_hint=(0.8, None), pos_hint={"top":1})
        #manager = ContenedorAssets()
        assCont = ContenedorAssets(asset.user)
        manager = Scrolling()
        #fold = ["unoaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","dos","tres",
...
if __name__ == "__main__":
    unaApp = AssetManApp()
    #unaApp.title = "Asset Manager ver.0.1"
    unaApp.run()